...
# Setup Django (lo script vive in data_migration_scripts/, la root del repo
# con manage.py/config/vendor_management_system è una directory sopra)
...
# Percorso relativo alla working directory: va lanciato dalla root del repo.
        ...
        for row in reader:
            try:
                ...
                if doc_type:
                    ...
                    doc_type.requires_renewal = parse_bool(
                        row["requires_renewal"]
                    )
                    doc_type.validity_period_days = (
                        parse_int(row["default_validity_days"]) or 365
                    )
                    doc_type.reminder_days_before = (
                        parse_int(row["alert_days_before_expiry"]) or 30
                    )
                    ...
                    doc_type.instructions = (
                        row["instructions"]
                        if row["instructions"] not in ["NULL", "", None]
                        else None
                    )
                    doc_type.save()
                    ...
                    doc_type = DocumentType(
                        ...
                        document_category=row["document_category"].strip()
                        if row["document_category"]
                        else "OTHER",
                        is_required=parse_bool(row["is_mandatory"]),
                        requires_renewal=parse_bool(row["requires_renewal"]),
                        validity_period_days=parse_int(
                            row["default_validity_days"]
                        )
                        or 365,
                        reminder_days_before=parse_int(
                            row["alert_days_before_expiry"]
                        )
                        or 30,
