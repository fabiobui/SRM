...
class TestUrlConfigView(View):
    ...
    def get(self, request):
        return JsonResponse(
            {
                "USE_FORNITORI_PREFIX": getattr(
                    settings, "USE_FORNITORI_PREFIX", None
                ),
                "FORCE_SCRIPT_NAME": getattr(
...
class SimpleTestView(View):
    ...
    def get(self, request):
        ...
        <p><strong>USE_FORNITORI_PREFIX:</strong>
        {getattr(settings, "USE_FORNITORI_PREFIX", "NON DEFINITO")}</p>
        <p><strong>FORCE_SCRIPT_NAME:</strong>
        {getattr(settings, "FORCE_SCRIPT_NAME", "NON DEFINITO")}</p>
