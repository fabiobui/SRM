...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not getattr(settings, "LDAP_ENABLED", False):
            self.stdout.write(
                self.style.WARNING(
                    "LDAP non è abilitato. Imposta LDAP_ENABLED=True "
                    "nel file .env"
                )
            )
            return
    ...
    def show_ldap_config(self):
        ...
        self.stdout.write(
            "   Server URI: "
            f"{getattr(settings, 'AUTH_LDAP_SERVER_URI', 'Non configurato')}"
        )
        self.stdout.write(
            "   Bind DN: "
            f"{getattr(settings, 'AUTH_LDAP_BIND_DN', 'Non configurato')}"
        )
        self.stdout.write(
            "   User Base DN: "
            f"{getattr(settings, 'LDAP_USER_BASE_DN', 'Non configurato')}"
        )
        self.stdout.write(
            "   Group Base DN: "
            f"{getattr(settings, 'LDAP_GROUP_BASE_DN', 'Non configurato')}"
        )
        self.stdout.write(
            f"   Start TLS: {getattr(settings, 'AUTH_LDAP_START_TLS', False)}"
        )
        self.stdout.write("")
    ...
    def get_ldap_connection(self):
        ...
        if use_ssl or getattr(settings, "AUTH_LDAP_START_TLS", False):
            try:
                ...
                # Se LDAP_TLS_VALIDATE=False, disabilita la validazione
                # certificato
                validate_cert = True
                if hasattr(settings, "LDAP_TLS_VALIDATE") and not getattr(
                    settings, "LDAP_TLS_VALIDATE", True
                ):
                    validate_cert = False
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "⚠️ ldap3 non installato. TLS potrebbe non "
                        "funzionare correttamente."
                    )
                )

        server = ldap3.Server(
            server_uri, get_info=ldap3.ALL, use_ssl=use_ssl, tls=tls_config
        )
    ...
    def test_connection(self):
        ...
        try:
            ...
            if hasattr(conn, "server") and hasattr(conn.server, "info"):
                self.stdout.write(
                    f"   Schema: {conn.server.info.naming_contexts}"
                )
        ...
        try:
            ...
            if success and conn.entries:
                for entry in conn.entries:
                    name = entry.cn.value
                    desc = (
                        entry.description.value if entry.description else "N/A"
                    )
    ...
    def test_authentication(self, email, password):
        ...
        try:
            ...
            if user:
                self.stdout.write(
                    self.style.SUCCESS("✅ Autenticazione riuscita!")
                )
                ...
                self.stdout.write(
                    "   Gruppi: "
                    f"{', '.join([g.name for g in user.groups.all()])}"
                )
            else:
                self.stdout.write(
