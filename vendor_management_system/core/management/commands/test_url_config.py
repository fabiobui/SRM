...
class Command(BaseCommand):
    help = (
        "Testa la configurazione degli URL per verificare il prefisso "
        "/fornitori"
    )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS("=== Test Configurazione URL ===\n")
        )
        ...
        if use_prefix:
            self.stdout.write(
                self.style.WARNING("🔧 Modalità PRODUZIONE attiva")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS("🚀 Modalità SVILUPPO attiva")
            )
        ...
        self.stdout.write("")
        self.stdout.write(
            "Per cambiare modalità, modifica USE_FORNITORI_PREFIX nel "
            "file .env"
        )
