...
class SimpleLDAPBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            ...
            user_base_dn = getattr(
                settings, "LDAP_USER_BASE_DN", "DC=sicura,DC=loc"
            )
            ...
            # Prima connessione: bind con account di servizio per
            # cercare l'utente
            ...
            user_attrs = {
                ...
                "displayName": str(user_entry.displayName)
                if user_entry.displayName
                else "",
                "mail": str(user_entry.mail) if user_entry.mail else username,
                "memberOf": [str(group) for group in user_entry.memberOf]
            ...
            bind_formats = [
                ...
                f"sicura\\{username.split('@')[0]}",  # Formato NetBIOS
                # domain\\username
            ]
            ...
            if not user_conn or not user_conn.bound:
                logger.warning(
                    f"Tutti i formati di bind falliti per: {username}"
                )
                return None
    ...
    def get_or_create_user(self, username, ldap_attrs):
        ...
        user.role = self.get_user_role_from_groups(
            ldap_attrs.get("memberOf", [])
        )
    ...
    def get_user_role_from_groups(self, member_of_groups):
        ...
        """
        # Mappatura gruppi LDAP -> ruoli (da configurare secondo i tuoi
        # gruppi AD)
        group_role_mapping = getattr(
            settings,
        ...
        for group_dn in member_of_groups:
            # Estrae il CN dal DN
            # (es. "CN=gruppo,OU=Groups,DC=domain,DC=com" -> "gruppo")
            if group_dn.startswith("CN="):
                cn_part = group_dn.split(",")[0]
                group_name = cn_part[
                    3:
                ].lower()  # Rimuove "CN=" e converte in minuscolo
...
class HybridAuthBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        logger.debug(
            f"Autenticazione LDAP fallita per {username}, "
            "provo con backend locale"
        )
