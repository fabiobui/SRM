...
creato **un set per classificazione**, così che nel tab "Requisiti
Professionali" del fornitore il dropdown proponga direttamente i requisiti
...
catalogo vengono creati, quelli esistenti aggiornati sui campi previsti dal
file.
...
COMPETENCES = [
    ...
    (
        "AMB-Cat.2bis",
        "Autorizzazione Trasporto rifiuti cat. 2Bis",
        "ENVIRONMENT",
        False,
        True,
        None,
    ...
    (
        "Lettera G DM37/2008",
        "Abilitazione Installazione Impianti Protezione Antincendio",
        "TECHNICAL",
        False,
        True,
        None,
    ...
    (
        "Saldatura",
        "Tecnici con patentini saldatura PED",
        "TECHNICAL",
        False,
        True,
        None,
...
COMPETENCE_SETS = [
    {
        ...
        "description": (
            "Requisiti professionali per i medici competenti "
            "(Medicina del Lavoro)."
        ),
    ...
    },
    {
        "name": "MDL - INFERMIERE",
        "description": (
            "Requisiti professionali per gli infermieri (Medicina del Lavoro)."
        ),
    ...
    },
    {
        ...
        "description": (
            "Requisiti professionali per gli infermieri di "
            "presidio. Il BLSD (MDL-R05) è obbligatorio per "
    ...
    },
    {
        ...
        "description": (
            "Requisiti professionali per i professionisti "
            "generici (Medicina del Lavoro)."
        ),
    ...
    },
    {
        "name": "SUBAPPALTATORI",
        "description": (
            "Requisiti professionali per i fornitori di tipo Subappaltatore."
        ),
        "category_code": "105",
        "codes": [
            "Patente a crediti",
...
class Command(BaseCommand):
    help = (
        "Popola il catalogo Requisiti Professionali e i Set Requisiti usati "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Riallinea al file anche i requisiti già presenti a catalogo. "
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.NOTICE("Inizio popolamento Requisiti Professionali...")
        )
        ...
        self.stdout.write(
            self.style.NOTICE(
                "Popolamento Set Requisiti Professionali completato."
            )
        )
    ...
    def _sync_sets(self, by_norm):
        ...
        for order, spec in enumerate(COMPETENCE_SETS, start=1):
            ...
            for code in spec["codes"]:
                ...
                (resolved if competence else missing).append(
                    competence or code
                )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{verb} set '{comp_set.name}': {len(resolved)} "
                    f"requisiti collegati."
                )
            )
            if missing:
                self.stdout.write(
                    self.style.WARNING(
                        f"  ⚠ Requisiti non trovati (ignorati): "
                        f"{', '.join(missing)}"
                    )
                )
