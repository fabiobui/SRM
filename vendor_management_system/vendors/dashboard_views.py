...
def vendor_dashboard_view(request):
    ...
    high_risk_vendors = vendors.filter(
    ...
    chart_data = {
        ...
        "by_ico_consultant": [
            {
                "is_ico": True,
                "count": vendors.filter(is_ico_consultant=True).count(),
            },
            {
                "is_ico": False,
    ...
    try:
        qualifiche_data = (
            Competence.objects.filter(vendor_assignments__is_qualifica=True)
            .annotate(
                vendor_count=Count(
                    "vendor_assignments__vendor",
                    filter=Q(
                        vendor_assignments__is_qualifica=True,
                        vendor_assignments__has_competence=True,
                    ),
                    distinct=True,
                )
            )
    ...
    try:
        competenze_data = (
            Competence.objects.filter(vendor_assignments__is_competenza=True)
            .annotate(
                vendor_count=Count(
                    "vendor_assignments__vendor",
                    filter=Q(
                        vendor_assignments__is_competenza=True,
                        vendor_assignments__has_competence=True,
                    ),
                    distinct=True,
                )
    ...
    try:
        service_categories_data = (
            ServiceType.objects.filter(parent__isnull=True)
            .annotate(
                vendor_count=Count(
                    "subservices__vendor_assignments__vendor", distinct=True
                )
            )
            .filter(vendor_count__gt=0)
            .values("name", "vendor_count")
    ...
    try:
        services_data = (
            ServiceType.objects.annotate(
                vendor_count=Count("vendor_assignments__vendor", distinct=True)
            )
    ...
    if not chart_data["by_services"]:
        chart_data["by_services"] = [
            {
                "service": "Nessun servizio assegnato",
                "count": 0,
                "category": None,
            }
        ]
    ...
    for range_label, min_val, max_val in quality_ranges:
        count = vendors.filter(
            quality_rating_avg__gte=min_val, quality_rating_avg__lt=max_val
        ).count()
    ...
    for range_label, min_val, max_val in fulfillment_ranges:
        count = vendors.filter(
            fulfillment_rate__gte=min_val, fulfillment_rate__lt=max_val
        ).count()
        chart_data["by_fulfillment"].append(
    ...
    for vendor in vendors:
        ...
        vendor_dict = {
            ...
            "category": {
                "name": vendor.category.name if vendor.category else None
            },
            "service_type": {
                "name": primary_service.service_type.name
                if primary_service
                else None,
                "parent": primary_service.service_type.parent.name
                if primary_service and primary_service.service_type.parent
                else None,
            }
            if primary_service
            else None,
...
def export_vendors_excel(request):
    ...
    for row, vendor in enumerate(vendors, 2):
        ...
        ws.cell(
            row=row,
            column=4,
            value=vendor.phone if hasattr(vendor, "phone") else "",
        )
        ws.cell(
            row=row,
            column=5,
            value=vendor.address.region if vendor.address else "",
        )
        ws.cell(
            row=row,
            column=6,
            value=vendor.address.state_province if vendor.address else "",
        )
        ws.cell(
            row=row,
            column=7,
    ...
    response["Content-Disposition"] = (
        "attachment; filename=fornitori_export.xlsx"
    )
