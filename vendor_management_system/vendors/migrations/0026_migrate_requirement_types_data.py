# Data migration: populate VendorCompetence type flags from
# Competence.requirement_type, then merge COMP/QUAL codes into REQ
...
def migrate_requirement_types(apps, schema_editor):
    ...
    # Step 1: Set boolean flags on VendorCompetence based on
    # Competence.requirement_type
    ...
    for number in set(comp_entries.keys()) & set(qual_entries.keys()):
        ...
        for vc in VendorCompetence.objects.filter(competence=qual_obj):
            ...
            if existing:
                ...
                if (
                    vc.certification_number
                    and not existing.certification_number
                ):
                ...
                if vc.issue_date and (
                    not existing.issue_date
                    or vc.issue_date > existing.issue_date
                ):
                    existing.issue_date = vc.issue_date
                if vc.expiry_date and (
                    not existing.expiry_date
                    or vc.expiry_date > existing.expiry_date
                ):
...
def reverse_migration(apps, schema_editor):
    ...
    # Set requirement_type based on first VendorCompetence found for
    # each competence
