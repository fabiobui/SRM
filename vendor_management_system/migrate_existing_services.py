"""
Script per migrare i servizi esistenti dal vecchio campo ForeignKey
service_type alla nuova relazione ManyToMany attraverso VendorService.
...
def migrate_services():
    ...
    print(
        colored(
            "\n📦 Migrazione servizi esistenti da service_type a "
            "VendorService",
            "cyan",
            attrs=["bold"],
        )
    ...
    with transaction.atomic():
        vendors_with_service = Vendor.objects.filter(
        ...
        for vendor in vendors_with_service:
            try:
                ...
                if created:
                    ...
                    print(
                        colored(
                            f"✓ [{migrated:3d}/{total}] Migrato: "
                            f"{vendor.name[:40]:40} → "
                            f"{vendor.service_type.name}",
                            "green",
                        )
                    )
                else:
                    ...
                    print(
                        colored(
                            f"⊘ [{skipped:3d}/{total}] Saltato "
                            f"(già esistente): {vendor.name[:40]:40}",
                            "yellow",
                        )
                    )
                ...
                errors += 1
                print(colored(f"✗ Errore per {vendor.name}: {str(e)}", "red"))
        ...
        print(
            colored(
                f"  📊 Totale elaborati:      {total}", "cyan", attrs=["bold"]
            )
        )
        print(colored("=" * 70 + "\n", "cyan"))
        ...
        if errors == 0:
            ...
            print(
                colored(
                    f"⚠️  Migrazione completata con {errors} errori",
                    "yellow",
                    attrs=["bold"],
                )
            )
