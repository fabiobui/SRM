...
(``albo_excel_row``) non viene toccato: resta il riferimento al file di
origine e viene riportato nel CSV di esito.

Uso:
    python vendor_management_system/import_embyon_codes.py --dry-run
    python vendor_management_system/import_embyon_codes.py
    python vendor_management_system/import_embyon_codes.py --prefix XLS \
        --report out.csv
...
Con --fuzzy i fornitori rimasti senza aggancio su C.F./P.IVA vengono cercati
per somiglianza della ragione sociale (soglia --fuzzy-threshold, default
0.90) usando la provincia come vincolo quando è nota da entrambe le parti.
"""
...
DRY_RUN = False
REPORT = "data_migration_scripts/reports/embyon_match_report.csv"
MAX_OMOCODIE = (
    3  # oltre questo numero di codici distinti non si sceglie da soli
)
...
# Forme societarie e titoli: tolti dal nome prima del confronto fuzzy,
# altrimenti "ROSSI SRL" e "ROSSI S.R.L." risulterebbero diversi e
# "ALFA SRL"/"BETA SRL" simili.
NOISE_TOKENS = {
    "SRL",
    "SRLS",
    "SPA",
...
# === HELPER FUNCTIONS ===
def norm_vat(value):
    """Normalizza una partita IVA: maiuscolo, senza separatori, senza
    prefisso IT."""
...
def norm_code(value):
    """Normalizza il CODCONTO: 'F     7' -> 'F 7' (come il modal di
    ricerca)."""
...
def similarity(a, b):
    ...
    quick_ratio() è un limite superiore di ratio(): se già non basta si evita
    il confronto completo, che su decine di migliaia di righe pesa parecchio.
    """
    if not a or not b:
        return 0.0
    la, lb = len(a), len(b)
...
def name_buckets(name):
    """Chiavi di blocking: prefisso di 4 caratteri di ogni parola
    significativa.

    Serve solo a restringere i candidati da confrontare; il match vero lo
    decide la similarità sull'intera ragione sociale.
    """
...
def load_embyon_rows(table):
    ...
    sql = (
        ...
        "  ON a.DITTA = f.DITTA AND a.CODCONTO = f.CODCONTO "
        "AND a.TIPOCONTO = 'F' "
        "WHERE f.TIPOCONTO = 'F'"
    )
    fallback = (
        "SELECT DITTA, CODCONTO, DSCCONTO1, PARTITAIVA, CODFISCALE, "
        f"Descrizione, NULL, NULL FROM {table} WHERE TIPOCONTO = 'F'"
    )
    with connection.cursor() as cursor:
        try:
            cursor.execute(sql)
        except Exception:
            # L'anagrafica con indirizzo può non esserci: si prosegue senza
            # provincia.
...
def find_fuzzy(vendor, by_name, threshold, require_province=False):
    ...
    if not best:
        nota = (
            f"scartati {scartati_prov} candidati di altra provincia"
            if scartati_prov
            else ""
        )
        return [], 0.0, nota
...
def find_candidates(vendor, by_vat, by_cf):
    ...
    for rec in (by_vat.get(vat, []) if vat else []) + (
        by_cf.get(cf, []) if cf else []
    ):
...
def pick_code(rows, prefer_ditta=None, max_omocodie=MAX_OMOCODIE):
    ...
    Se lo stesso fornitore esiste su più Società (più CODCONTO distinti)
    viene preso quello con il numero più alto dopo la 'F', cioè il codice
    più recente.
    La scelta automatica vale però solo fino a ``max_omocodie`` codici
    distinti: oltre quella soglia il fornitore è troppo frammentato in
    anagrafica (es. i gruppi con la stessa P.IVA su decine di sedi) e va
    deciso a mano.
    ...
    if prefer_ditta:
        filtered = [
            r for r in rows if r["ditta"].lower() == prefer_ditta.lower()
        ]
        if filtered:
            rows = filtered
    ...
    if len(distinct) > max_omocodie:
        return None, (
            f"ambiguo: {len(distinct)} omocodie (oltre il limite di "
            f"{max_omocodie}): {', '.join(codes)}"
        )

    scartati = ", ".join(
        c for c in codes if code_key(c) != code_key(chosen["code"])
    )
    return (
        chosen,
        f"ambiguo ({len(distinct)} omocodie): scelto il maggiore, "
        f"scartati {scartati}",
    )


def import_embyon_codes(
    prefix=None,
    dry_run=None,
    report=None,
    prefer_ditta=None,
    ...
    table = getattr(
        dj_settings,
        "EMBYON_FORNITORI_TABLE",
        "redmine_test.Embyon_Fornitori_T",
    )
    ...
    try:
        ...
        print(
            colored(
                f"❌ Errore leggendo l'anagrafica Embyon ({table}): {exc}",
                "red",
            )
        )
        return
    ...
    qs = (
        Vendor.objects.filter(old_code__startswith=prefix)
    ...
    print(
        colored(
            f"📦 Anagrafica Embyon: {table} | "
            f"righe TIPOCONTO='F': {total_rows}",
            "cyan",
            attrs=["bold"],
        )
    )
    print(
        colored(
            f"   Fornitori con old_code '{prefix}*': {len(vendors)} | "
            f"DRY_RUN: {dry_run}",
            "cyan",
            attrs=["bold"],
        )
    ...
    print(
        colored(
            "   Scelta automatica del codice maggiore fino a "
            f"{max_omocodie} omocodie",
            "cyan",
        )
    )
    if fuzzy:
    ...
    if prefer_ditta:
        print(
            colored(
                f"   Società preferita in caso di ambiguità: {prefer_ditta}",
                "cyan",
            )
        )
    print()
    ...
    unknown = {
        r["ditta"] for recs in by_vat.values() for r in recs if r["ditta"]
    } - known_companies
    if unknown:
        print(
            colored(
                "   ⚠️  Società presenti su Embyon ma non in "
                "Vendor.EMBYON_COMPANY_CHOICES: "
                f"{', '.join(sorted(unknown))}",
                "yellow",
            )
        )

    # Codici già assegnati ad altri vendor: old_code è UNIQUE, va evitata
    # la collisione.
    taken = {
        code_key(c): pk
    ...
    with transaction.atomic():
        for i, vendor in enumerate(vendors):
            # Riferimento alla riga dell'Albo: old_code sta per essere
            # sostituito, quindi è l'unico aggancio che resta verso il
            # file di origine.
            riga = (
                f" (riga Albo {vendor.albo_excel_row})"
                if vendor.albo_excel_row
                else ""
            )
            ...
            if not candidates and fuzzy:
                # Nessun aggancio su C.F./P.IVA: si ripiega sulla ragione
                # sociale.
            ...
            if chosen is None:
                if candidates:
                    ...
                    print(
                        colored(
                            f"[{i + 1}] ⚠️  {vendor.old_code}{riga} "
                            f"{vendor.name[:40]} — {note}",
                            colore,
                        )
                    )
                else:
                    ...
                    print(
                        colored(
                            f"[{i + 1}] ⚪ {vendor.old_code}{riga} "
                            f"{vendor.name[:40]} — nessun match",
                            "white",
                        )
                    )
                rows_report.append(
                    [
                        vendor.pk,
                        vendor.albo_excel_row or "",
                        vendor.old_code,
                        "",
                        vendor.name,
                        vendor.fiscal_code or "",
                        vendor.vat_number or "",
                        criterion or "",
                        "",
                        "",
                        "",
                        sim,
                        vendor_province(vendor),
                        "",
                        esito,
                        note or "",
                    ]
            ...
            if owner and owner != vendor.pk:
                skipped += 1
                other = (
                    Vendor.objects.filter(pk=owner)
    ...
    print(
        colored(
            f"   Aggiornati: {updated} (di cui {ambiguous} con più codici, "
            "preso il maggiore"
            + (f"; {fuzzy_hits} per ragione sociale" if fuzzy_hits else "")
            + ") "
            f"| Saltati: {skipped} | Senza match: {not_found}",
            "green",
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Associa il Codice Embyon (old_code) ai fornitori "
            "cercandoli per C.F./P.IVA"
        )
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        ...
        default=REPORT,
        help=(
            "File CSV con l'esito riga per riga (contiene anche i "
            "codici precedenti)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Aggiorna anche il flag 'Attivo su Embyon' in base allo "
            "stato Embyon"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Se C.F./P.IVA non agganciano, cerca per somiglianza della "
            "ragione sociale"
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Similarità minima 0..1 per il match fuzzy (default: "
            f"{FUZZY_THRESHOLD})"
        ),
    )
    parser.add_argument(
        "--fuzzy-province-required",
        dest="fuzzy_province_required",
        action="store_true",
        help=(
            "Accetta un match fuzzy solo se la provincia è nota e "
            "coincide su entrambi i lati"
        ),
    )
    parser.add_argument(
        "--max-omocodie",
        dest="max_omocodie",
        type=int,
        default=MAX_OMOCODIE,
        help=(
            "Numero massimo di codici distinti entro cui scegliere il "
            f"maggiore (default: {MAX_OMOCODIE})"
        ),
    )
    parser.add_argument(
        "--limit",
    ...
    import_embyon_codes(
        ...
        report=args.report,
        prefer_ditta=args.prefer_ditta,
        limit=args.limit,
        set_active=args.set_active,
        max_omocodie=args.max_omocodie,
        fuzzy=args.fuzzy,
