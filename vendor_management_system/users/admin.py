...
class CustomUserCreationForm(UserCreationForm):
    ...
    vendor = forms.ModelChoiceField(
        ...
        label=_("Fornitore"),
        help_text=_(
            "Associa l'utente a un fornitore (solo per utenti vendor)"
        ),
        widget=forms.Select(attrs={"class": "form-control"}),
    )
...
class CustomUserChangeForm(UserChangeForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            self.fields["user_type"].initial = (
                "ldap" if self.instance.is_ldap_user else "local"
            )
...
class UserAdmin(auth_admin.UserAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Permessi"),
            {
                ...
                "description": _(
                    "Per un utente con Ruolo 'Fornitore' non serve toccare "
                    "Gruppi né Privilegi utente: l'accesso al portale "
                    "dipende solo da Ruolo e Fornitore (sopra), non da "
                    "questi campi. Il gruppo 'Vendors' viene assegnato "
                    "automaticamente alla creazione. Gruppi/Privilegi "
                    "utente servono solo per gli utenti Admin/BO con "
                    "'Staff status' attivo, per limitarne l'accesso a "
                    "specifiche viste di /admin/ invece di renderli "
                    "superuser."
                ),
            },
        ),
    ...
    add_fieldsets = (
        ...
        (
            _("Tipo di Autenticazione"),
            {
                ...
                "fields": ("user_type",),
                "description": _(
                    "Seleziona 'LDAP' se l'utente si autentica tramite "
                    "Active Directory"
                ),
            },
        ),
        ...
        (
            ...
            {
                # Niente "collapse": è un campo obbligatorio per gli utenti
                # locali (vedi CustomUserCreationForm.clean()) e non va
                # nascosto di default dietro un toggle "Mostra".
                "classes": ("wide",),
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        # Per utenti LDAP esistenti, alcune informazioni non dovrebbero
        # essere modificabili
        if obj and obj.is_ldap_user:
            readonly.extend(
                ["email", "name"]
            )  # Questi vengono sincronizzati da LDAP
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        In creazione (`obj is None`) va sempre usato `add_fieldsets`, con
        `password1`/`password2`: `fieldsets` mostra invece `password` in
        sola visualizzazione, pensato per la modifica di un utente già
        esistente.
        """
        if obj is None:
    ...
    class Media:
        ...
        css = {"all": ("admin/css/ldap_user_admin.css",)}
