...
class AdminDashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        vendors_with_docs = (
            Vendor.objects.filter(documents__isnull=False).distinct().count()
        )
...
class VendorPortalView(VendorRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        existing_types = vendor_documents.values_list(
        ...
        expiring_docs = []
        for doc in vendor_documents.filter(
            status__in=["APPROVED", "UPLOADED"]
        ):
            if doc.is_expiring_soon:
...
class DocumentUploadView(VendorRequiredMixin, View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.success(
                request,
                f"Documento {document_type.name} caricato con successo!",
            )
...
class DocumentReviewView(BackOfficeRequiredMixin, View):
    ...
    def post(self, request, document_id):
        ...
        if action == "approve":
            ...
            messages.success(
                request, f"Documento {document.document_type.name} approvato!"
            )
        elif action == "reject":
            document.status = "REJECTED"
            messages.warning(
                request, f"Documento {document.document_type.name} rifiutato!"
            )
