...
# Nel file lo stesso requisito compare sotto più codici (QUAL-001 e COMP-001
# sono entrambi "Qualifica RSPP"): il prefisso non identifica il requisito,
# dice con quale ruolo viene assegnato al fornitore. Per questo i flag
# stanno su VendorCompetence e non su Competence.
TYPE_BY_PREFIX = {
    "QUAL": "is_qualifica",
    "COMP": "is_competenza",
...
# Corrispondenze da codice colonna del file a codice del catalogo, per i
# casi in cui la descrizione nel file non coincide con il nome a catalogo e
# l'aggancio automatico non riesce. Si può aggiungerne una al volo con
# --alias.
ALIASES = {
    # "QUAL-003": "REQ-003",   # Qualifica Coord. Sicurezza Cantieri ->
...
def norm_name(value):
    """Normalizza una descrizione per il confronto: minuscolo, spazi
    collassati."""
...
def assignment_flag(code):
    """Flag di VendorCompetence corrispondente al prefisso del codice
    colonna."""
...
def build_competence_index():
    """Indici del catalogo a DB: per codice normalizzato e per nome
    normalizzato.
...
def import_competences(
    ...
    print(
        colored(
            f"   Righe dati: {len(df)} | "
            f"Colonne requisito: {len(code_to_name)}",
            "cyan",
            attrs=["bold"],
        )
    )
    ...
    resolved_cols = {}  # codice colonna -> (Competence, flag) o (None, flag)
    ...
    print(
        colored(
            f"   Requisiti riconosciuti a catalogo: "
            f"{noti}/{len(resolved_cols)}",
            "cyan",
        )
    )
    for code, (comp, _flag) in resolved_cols.items():
        if not comp:
            print(
                colored(
                    f"   ⚠️  {code} ({code_to_name[code][:40]}) "
                    "non è a catalogo"
                    + (
                        ""
                        if create_missing
                            ...
                            " — colonna ignorata "
                            "(usa --alias CODICE=CODICE_CATALOGO)"
                        )
                    ),
                    "yellow",
                )
    ...
    if senza_tipo:
        print(
            colored(
                f"   ⚠️  Prefisso non riconosciuto (nessun tipo assegnabile): "
                f"{', '.join(senza_tipo)}",
                "yellow",
            )
        )
    ...
    with transaction.atomic():
        # Azzeramento delle assegnazioni dei fornitori di altre Società, da
        # fare prima dell'import per non lasciare righe di un caricamento
        # precedente.
        if reset_except_company:
            ...
            print(
                colored(
                    ...
                    f"{reset_except_company}, vuota compresa): {deleted}",
                    "yellow",
                    attrs=["bold"],
                )
            )
        ...
        for i, row in df.iterrows():
            ...
            for col, val in row.items():
                ...
                comp, flag = resolved_cols.get(
                    comp_code, (None, assignment_flag(comp_code))
                )

                if comp is None:
                    if not create_missing:
                        # Il requisito non è a catalogo: crearlo qui
                        # produrrebbe doppioni dei codici storici, quindi
                        # si salta.
                        continue
                ...
                # Il tipo dipende dalla colonna: lo stesso requisito può
                # arrivare come qualifica e come competenza, e allora la
                # riga è una sola con entrambi i flag.
                tipo = ""
                ...
                if created_vc:
                    ...
                    print(
                        colored(
                            f"   ✅ Assegnato: {comp.name}{tipo}", "yellow"
                        )
                    )
                elif tipo:
                    ...
                    print(
                        colored(
                            f"   ➕ Tipo aggiunto: {comp.name}{tipo}", "yellow"
                        )
                    )
                else:
    ...
    if reset_except_company:
        print(
            colored(
                f"   Assegnazioni rimosse prima dell'import: {deleted}",
                "yellow",
            )
        )
    print(
        colored(
            f"   Assegnazioni create: {created_vendor_competences}", "green"
        )
    )
    print(
        colored(
            f"   Assegnazioni a cui è stato aggiunto un tipo: "
            f"{updated_vendor_competences}",
            "green",
        )
    )
    print(
        colored(
            f"   Requisiti creati a catalogo: {created_competences}", "green"
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Import competenze vendor da Excel/CSV"
    )
    parser.add_argument(
        "-f",
        "--file",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Crea a catalogo i requisiti non riconosciuti (default: li salta)"
        ),
    )
    parser.add_argument(
        "--alias",
        action="append",
        default=[],
        metavar="COL=CATALOGO",
        help=(
            "Aggancia una colonna a un requisito a catalogo "
            "(es. QUAL-003=REQ-003); ripetibile"
        ),
    )
    parser.add_argument(
        "--reset-except-company",
        dest="reset_except_company",
        default=None,
    ...
    for item in args.alias:
        if "=" not in item:
            print(
                colored(
                    f"❌ Alias non valido (atteso COL=CATALOGO): {item}", "red"
                )
            )
            sys.exit(1)
    ...
    import_competences(
        ...
        reset_except_company=args.reset_except_company,
        aliases=alias_map,
        key=args.key,
    )
